    ...
    <div>
        <div id="servers-container">
            <v-list id="servers-btn">
                <v-list-tile @click="servers_visible = !servers_visible">
                    <v-list-tile-action>
                        <v-icon color="white">computer</v-icon>
                    </v-list-tile-action>
                    <v-list-tile-content>
                        <v-list-tile-title>Servers</v-list-tile-title>
                    </v-list-tile-content>
                    <v-list-tile-action v-if="!servers_visible">
                        <v-icon color="white">arrow_drop_down</v-icon>
                    </v-list-tile-action>
                    <v-list-tile-action v-if="servers_visible">
                        <v-icon color="white">arrow_drop_up</v-icon>
                    </v-list-tile-action>
                </v-list-tile>
            </v-list>
        </div>
        ...
        <div v-if="servers_visible">
            <v-list two-line>
                <v-list-tile v-for="server in servers" :key=server.id @click="selectServer(server)" class="server ">
                    ...
                    <v-list-tile-action v-if="server == activeServer">
                        <v-icon>done</v-icon>
                    </v-list-tile-action>
                    <v-list-tile-action v-if="server != activeServer && (hostServer != null && server != hostServer)"
            ...
            <div>
                <v-btn outline class="server-btn" style="color: grey;" @click="clearServersDialog = true">
                    <v-icon>clear_all</v-icon>&ensp;clear
                </v-btn>
                <v-btn outline class="server-btn" style="color: #26a69a;" @click="addServerDialog = true">
                ...
                <v-dialog v-model="addServerDialog" width='400px'>
                    <div>
                        <server-input-field @addServerFinish="addServerFinishHandler()"
                                            @cancelAddServer="cancelAddServerHandler()"
                                            :enableCancel="true"></server-input-field>
                    </div>
                </v-dialog>
                ...
                <v-dialog v-model="clearServersDialog" width="400px">
                    <v-card>
                        ...
                        <v-card-actions>
                            <confirm-btns leftPrimary rightMinor leftPrependVIcon="exit_to_app"
                                          rightPrependVIcon="clear_all" rightText="Clear"
                                          @clickLeft='clearServersDialog=false' @clickRight="clearServers()"/>
                        </v-card-actions>
                    </v-card>
                </v-dialog>
            </div>
        ...
        <v-list v-if="!servers_visible">
            ...
            <v-list-tile @click="showProcesses">
                ...
                <v-list-tile-content>
                    <v-list-tile-title>Processes</v-list-tile-title>
                </v-list-tile-content>
            </v-list-tile>
            ...
            <v-list-tile @click="showFS">
                ...
                <v-list-tile-content>
                    <v-list-tile-title>File Systems</v-list-tile-title>
                </v-list-tile-content>
            </v-list-tile>

            <v-list-tile @click="showReport">
                <v-list-tile-action>
                    <icon>harddisk</icon>
                </v-list-tile-action>
                <v-list-tile-content>
                    <v-list-tile-title>Show Report</v-list-tile-title>
                </v-list-tile-content>
            </v-list-tile>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            showReport() {
                cm.bus.$emit('changeContent', 'report')
            }
